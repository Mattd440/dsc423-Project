...
full_model = lm(data=test_df, formula=log(Chance.of.Admit/(1-Chance.of.Admit)) ~ . )
summary(full_model)
vif(full_model)

plotResiduals(test_df, full_model)
...
```{r}
base = lm(data=test_df, formula = Chance.of.Admit ~ 1)
...
model2 = lm(data=test_df, formula = log(Chance.of.Admit/(1-Chance.of.Admit)) ~CGPA + GRE.Score + LOR + Research )
summary(model2)
```
...
model3 = lm(data=test_df, formula=log(Chance.of.Admit/(1-Chance.of.Admit))~(CGPA+ GRE.Score + LOR + Research)^2)
summary(model3)
...
model4 = lm(data=test_df, formula=log(Chance.of.Admit/(1-Chance.of.Admit))~CGPA+GRE.Score+LOR+Research + Research:CGPA + Research:GRE.Score +Research+ Research:LOR)
summary(model4)
```

# Remove Insignificant Features
```{r}
model5= lm(data=test_df, formula=log(Chance.of.Admit/(1-Chance.of.Admit))~CGPA+GRE.Score+LOR+Research + Research:CGPA )
summary(model5)
```

#PLOT RESIDUALS
```{r}
plotResiduals(test_df, model5)
...
ggplot(data=test_df, aes(x=hatvalues(model5), y=rstudent(model5)))+
  geom_point() + 
...
outlierindexs = c(10,11,65,66,69)
grad_df_noOutliers =test_df[-outlierindexs, ]

model6= lm(data=grad_df_noOutliers, formula=log(Chance.of.Admit/(1-Chance.of.Admit))~CGPA+GRE.Score+LOR+Research + Research:CGPA )
summary(model6)
...
cv_model = train(log(Chance.of.Admit/(1-Chance.of.Admit))~CGPA+GRE.Score+LOR+Research+ Research:CGPA  ,data=grad_df_noOutliers, method='lm', trControl = cv_control)
